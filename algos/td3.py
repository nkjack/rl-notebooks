...
Input:
    ...
    - Q-function network - q_network1, q_network2
    - empty replay buffer D - replay_buffer
    - target for policy + Q-function - p_target, q_target1, q_target2
    
Repeat
    observe state
    select action = clip(mu(s) + eps, a_low, a_high), where eps ~ N
    ...
    is it time to update ?
        for update in num_of_updates:
            ...
            # compute targets
            act = np.clip(p_target(new_states) + eps, a_low, a_high)
            y = rewards + delta * ( 1 - is_dones ) * min_i (q_target_i(new_states, act))
            
            # update Q-function by one step gradient descent
            loss_n1 = F.MSE(q_network1(states, actions), y)
            loss_n2 = F.MSE(q_network2(states, actions), y)
...
class ActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, hidden_sizes=(256,256),
        ...
        self.DQN1 = DQN(obs_dim, act_dim, hidden_sizes, activation)
        self.DQN2 = DQN(obs_dim, act_dim, hidden_sizes, activation)
...
def td3(env_fn, actor_critic=ActorCritic, ac_kwargs=dict(), seed=0,
        ...
        max_ep_len=1000, save_freq=1, policy_delay=2, target_noise=0.2):
    ...
    q_params = itertools.chain(ac.DQN1.parameters(), ac.DQN2.parameters())
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            pi_target = ac_targ.actor(o2)
            eps = torch.randn_like(pi_target) * target_noise
            action2 = torch.clamp(pi_target + eps, -act_limit, act_limit)

            q_pi_targ1 = ac_targ.DQN1(o2, action2)
            q_pi_targ2 = ac_targ.DQN2(o2, action2)
            q_pi_targ = torch.min(q_pi_targ1, q_pi_targ2)
    ...
    def update(data, update_policy=False):
        ...
        if update_policy:
            ...
            for p in ac.DQN1.parameters():
                p.requires_grad = False

            for p in ac.DQN2.parameters():
                p.requires_grad = False
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            for j in range(update_every):
                ...
                if j % policy_delay == 0:
                    update(data=batch, update_policy=True)
                else:
                    update(data=batch, update_policy=False)
...
if __name__ == '__main__':
    ...
    print('\nUsing td3 formulation.\n')

    td3(lambda : gym.make(args.env), actor_critic=ActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l),
        gamma=args.gamma, seed=args.seed, epochs=args.epochs)


# example
## python td3.py --env LunarLanderContinuous-v2 --start_steps 500