...
def train(env_name='CartPole-v0', hidden_sizes=[32], lr=1e-2, epochs=50, batch_size=5000, render=False, gamma=0.99,
          lam=0.95, adv='vnl', clip_ratio=0.2):
    ...
    logits_net_actions = mlp([obs_dim] + hidden_sizes + [n_acts])
    ...
    def compute_loss(obs, acts, logp_old, adv):
    ...
    def train_one_epoch(epoch_num):
        ...
        while True:
            ...
            # calculate step
            action, logp = get_action(torch.as_tensor(obs, dtype=torch.float32))
        ...
        logp_old = torch.as_tensor(batch_logp, dtype=torch.float32)

        ## optimize policy
        optimizer_policy.zero_grad()

        # calculate loss
        loss_policy = compute_loss(obs=obs, acts=act, logp_old=logp_old, adv=adv)
