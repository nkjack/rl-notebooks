...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim, hid_sizes: List[int], activation, act_limit):
        super(Actor, self).__init__()
        self.net = mlp([obs_dim] + hid_sizes, activation, activation)
        self.mu_layer = nn.Linear(hid_sizes[-1], action_dim)
        self.log_std_layer = nn.Linear(hid_sizes[-1], action_dim)
        self.act_limit = act_limit

    def forward(self, obs, deteministic=False, with_logprob=True):
...
def sac(env_fn, actor_critic=ActorCritic, ac_kwargs=dict(), seed=0,
...
if __name__ == '__main__':
    ...
    print('\nUsing sac formulation.\n')

    sac(lambda: gym.make(args.env), actor_critic=ActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid] * args.l),
        gamma=args.gamma, seed=args.seed, epochs=args.epochs)

# example
## python sac.py --env LunarLanderContinuous-v2 --start_steps 500